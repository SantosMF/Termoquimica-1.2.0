...
#--------- comandos para tradução de interface de dados ----------------------
    E_eletronic = 'Energia eletrônica'
    ponto_zero = 'Energia de ponto zero'
    functions = 'Funções termodinâmicas'
#-----------------------------------------------------------------------------
...
#--------------------------- se gás monoatômico ------------------------------
    ...
    with open(path+'/temps/temp.nyp', 'w') as temp:
                temp.write(str(datetime.now().strftime('#Criado em %d/%m/%Y às %H:%M:%S'))+'\n')
                temp.write(result)
    return result
